...
class BigNum {
 public:
  ...
  BigNum(const int64_t x) {
    len = 0;
    sign = x < 0 ? -1 : 1;
    std::memset(elem, 0, MAX_SIZE * sizeof(int));
    for (int64_t i = std::abs(x); i; i /= BigNum::base)
  ...
  // index
  int& operator[](const int index) { return elem[index]; }

  int operator[](const int index) const { return elem[index]; }
  ...
  // compare
  bool operator<(const BigNum& other) const { return cmp(*this, other) < 0; }
  ...
  BigNum operator+(const int64_t smallNum) const {
  ...
  BigNum& operator+=(const int64_t smallNum) {
  ...
  friend int cmp(const BigNum& a, const BigNum& b, bool isAbs = false) {
    ...
    for (int i = a.len - 1; i >= 0; i--)
      if (a[i] != b[i]) return a[i] > b[i] ? a.sign : -a.sign;
    return 0;
  }
  ...
  friend BigNum add(const BigNum& a, const BigNum& b) {
    ...
    for (int i = 0; i < ret.len; i++) ret[i] = a[i] + b[i];
  ...
  friend BigNum& abs(const BigNum& a) { return a.abs(); }

  std::string toString() const {
    std::string s;
    if (sign < 0) s += "-";
    for (int i = len - 1; i >= 0; i--) s += std::to_string(elem[i]);
    return s;
  }

  friend std::string toString(const BigNum& src) { return src.toString(); }
